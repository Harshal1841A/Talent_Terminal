...
def load_artifacts(skip_bm25: bool = False):
    ...
    if not skip_bm25 and bm25_path.exists() and bm25_path.is_dir():
        try:
            import bm25s
            bm25 = bm25s.BM25.load(str(bm25_path), load_corpus=True)
            bm25_ids = {cid["text"]: i for i, cid in enumerate(bm25.corpus)}
            print(f"[INFO] Loaded BM25s index ({len(bm25_ids):,} docs)")
        except MemoryError:
            print(
                "[WARN] Not enough RAM to load bm25_index — "
                "continuing without BM25 (FAISS+CE only). "
                "Use --no-bm25 to suppress this warning."
            )
            bm25, bm25_ids = None, {}
...
def main():
    ...
    parser.add_argument("--use-mmr", action="store_true", help="Enable MMR during tuning")
    parser.add_argument("--no-bm25", action="store_true", help="Skip loading BM25 index (saves RAM; uses FAISS+CE only)")
    parser.add_argument(
        "--objective",
        default="composite",
    ...
    metadata, faiss_index, bm25, bm25_ids, lgb_model = load_artifacts(skip_bm25=args.no_bm25)
    bi_enc, cross_enc = load_models(args.offline)

    base_cfg = RankingConfig(apply_mmr=args.use_mmr)
    ...
    if finetuned_ce_path.exists():
        ...
        cross_enc_path = str(BASE / "models" / "ms-marco-MiniLM-L-6-v2") if args.offline else "cross-encoder/ms-marco-MiniLM-L-6-v2"
    ...
    if best_path.exists():
        ...
        existing_ndcg = existing.get("metrics", {}).get(f"ndcg@{args.k}", 0.0)
        new_ndcg = payload.get("metrics", {}).get(f"ndcg@{args.k}", 0.0)

        if new_ndcg <= existing_ndcg:
            print(f"\nNew result (ndcg@{args.k}={new_ndcg:.4f}) does not beat existing "
                  f"best_weights.json (ndcg@{args.k}={existing_ndcg:.4f}). Not overwriting.")
            print("To force overwrite anyway, delete best_weights.json first.")
            exit(0)
        else:
            print(f"\nNew result improves ndcg@{args.k}: {existing_ndcg:.4f} -> {new_ndcg:.4f}")
