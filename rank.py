...
def rank_candidates():
    ...
    k_retrieve = min(800, len(metadata))
    ...
    # Slice to top 500 for Cross-Encoder re-ranking as per methodology
    top_k_indices = top_k_indices_array[0].tolist()[:500]
    ...
    # batch_size=8 strictly caps peak RAM usage to avoid OOM crashes on tight VMs
    cross_scores = cross_enc.predict(cross_inputs, batch_size=8)
    ...
    # 8. Write submission.csv using team name from metadata
