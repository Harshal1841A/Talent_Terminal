...
def norm_semantic(raw_logit: float) -> float:
    ...
    CrossEncoder (ms-marco-MiniLM-L-6-v2) logits typically range from -10 to +10.
    A sigmoid centered near 0 smoothly maps these to 0-1, 
    scaled by W_SEMANTIC.
    """
...
def score_engagement(meta: dict, w_engagement: float = W_ENGAGEMENT) -> float:
    ...
    if apps >= 5: score += 1.5
    elif apps >= 2: score += 0.8
    elif apps == 0: score -= 0.5

    searches = meta.get("search_appearance_30d", 0) or 0
    if searches >= 200: score += 1.0
    elif searches >= 50: score += 0.5

    resp_hrs = meta.get("avg_response_time_hours", 999) or 999
    if resp_hrs <= 4: score += 1.0
    elif resp_hrs <= 24: score += 0.5
    elif resp_hrs >= 168: score -= 1.0

    endorsements = meta.get("endorsements_received", 0) or 0
    if endorsements >= 50: score += 0.5
...
def score_behavioral(meta: dict, w_behavioral: float = W_BEHAVIORAL) -> float:
    ...
    last_active_days = meta.get("last_active_days", 365) or 365
...
def generate_reasoning(meta: dict, semantic_score: float, final_score: float) -> str:
    ...
    _hash = sum(ord(c) for c in cid) % 4
    ...
    if loc >= 4.0:
        lead_parts.append(f"Location is a direct match for the JD's preferred cities (score: {loc:.0f}/5) — no relocation friction expected.")
    elif ratio >= 0.65 and ml_mo >= 36:
        yrs_ml = ml_mo // 12
        lead_parts.append(f"{yrs_ml} of their {yrs:.0f} total years ({ratio:.0%}) were spent in ML-titled roles — career composition closely matches the JD's '4-5 of 6-8 years in applied ML' target.")
    elif ml >= 0.8:
        openers = ["Strong production ML depth across their career — ", "Career history shows consistent production ML work — ", "Production ML signals are strong throughout their history — ", "Demonstrated production ML track record — "]
        lead_parts.append(openers[_hash] + "retrieval, ranking, or embedding systems detected in role descriptions.")
    elif saved >= 8:
        lead_parts.append(f"High recruiter demand signal: {saved} recruiters have bookmarked this profile in the last 30 days — already being actively competed for.")
    elif rfs == 15.0 and ml >= 0.3:
        lead_parts.append("Research/founding background with production deployment signals — the research-plus-ship profile this JD values.")
    elif final_score >= 80:
        openers = ["Across the board strong profile for this JD — ", "Multiple signals align with what the JD is looking for — ", "Well-rounded match for this role — ", "Profile consistently scores well across the JD's criteria — "]
        lead_parts.append(openers[_hash] + "details below.")

    if 5 <= yrs <= 9: secondary_parts.append(f"{yrs:.0f} years total experience, squarely in the 5-9yr band.")
    elif 4 <= yrs < 5: secondary_parts.append(f"{yrs:.0f} years experience — just below the 5yr threshold, still in scope.")
    elif 9 < yrs <= 12: secondary_parts.append(f"{yrs:.0f} years experience — slightly senior, but within reasonable range for this role.")
    elif yrs < 4: concern_parts.append(f"Only {yrs:.0f} years experience — below the 5yr minimum; junior risk.")
    else: concern_parts.append(f"{yrs:.0f} years experience — significantly senior; may be overqualified or command above-budget comp.")
    ...
    if ratio >= 0.5 and not any("career composition" in p for p in lead_parts):
        ...
        concern_parts.append(f"Only {ratio:.0%} of career in ML-titled roles — most of their experience was in other domains.")

    if meta.get("consulting_only"):
        concern_parts.append(f"Consulting-only background ({company}) — JD explicitly warns this is disqualifying without product-company experience.")
    elif meta.get("has_product_company"):
    ...
    if ml >= 0.5 and not any("production ML" in p.lower() for p in lead_parts):
        secondary_parts.append(f"Moderate-to-strong production ML signals in career descriptions (score: {ml:.2f}).")
    elif ml < 0.2 and not any("production ML" in p.lower() for p in lead_parts):
        concern_parts.append("No meaningful production ML/retrieval signals found in role descriptions — skills may be self-reported without deployment experience.")

    if rfs == 15.0:
        if ml >= 0.3: secondary_parts.append("Research/Applied Scientist background with production ML signals — the research-to-production profile this JD values.")
        else: concern_parts.append("Research Scientist title but minimal production deployment signals — may be a pure-research profile, which the JD explicitly flags as a non-fit.")
    elif rfs == -5.0:
        concern_parts.append("Intern/student-only career history — insufficient professional experience.")

    if chaser:
        concern_parts.append(f"Career shows rapid seniority escalation with avg {avg_t:.0f} months per role — matches the JD's explicit concern about candidates optimizing for titles over tenure.")
    ...
    if loc >= 4.0 and not any("Location" in p for p in lead_parts):
        ...
        concern_parts.append("Located outside JD's preferred cities (Pune/Noida) without a relocation signal.")

    if meta.get("skill_count", 0) > 80 and yrs < 3:
        concern_parts.append(f"Profile lists {meta.get('skill_count')} skills with only {yrs:.0f} years experience — pattern consistent with keyword inflation.")

    if saved >= 10 and not any(str(saved) in p for p in lead_parts):
        secondary_parts.append(f"Bookmarked by {saved} recruiters in 30 days — high market demand.")
    ...
    if gh >= 70: secondary_parts.append(f"Active GitHub presence ({gh:.0f}/100) — public engineering output visible.")
    elif gh >= 30: secondary_parts.append(f"Moderate GitHub activity ({gh:.0f}/100).")

    if meta.get("core_skill_score", -1) >= 80:
        secondary_parts.append(f"Scored {meta['core_skill_score']:.0f}/100 on the platform's core ML assessment.")
    ...
    avail = []
    if meta.get("open_to_work"): avail.append("open to work")
    if nd == 0: avail.append("immediate joiner")
    elif nd <= 30: avail.append(f"{nd}-day notice (buyable per JD)")
    elif nd > 90: concern_parts.append(f"{nd}-day notice period — long lead time, flag for hiring timeline.")
    if rr >= 0.75: avail.append(f"{rr:.0%} response rate")
    elif rr < 0.3: concern_parts.append(f"Low recruiter response rate ({rr:.0%}) — reachability risk.")
    if 0 <= lad <= 7: avail.append("active this week")
    elif lad > 180: concern_parts.append(f"Inactive on platform for {lad} days — engagement unclear.")
    if avail: secondary_parts.append("Availability: " + ", ".join(avail) + ".")
